...
class Ion_auth_model extends CI_Model
{
	...
	{
		$this->ion_auth_model->trigger_events('pre_register');
		$email_activation = $this->config->item('email_activation', 'ion_auth');
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login,level')
		                  ->where(array($this->identity_column => $identity, level => '0'))
